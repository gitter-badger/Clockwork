...
#include <Clockwork/Clockwork2D/CollisionCircle2D.h>
#include <Clockwork/Clockwork2D/CollisionEdge2D.h>
#include <Clockwork/Clockwork2D/CollisionPolygon2D.h>
#include <Clockwork/Clockwork2D/ConstraintDistance2D.h>
#include <Clockwork/Clockwork2D/ConstraintFriction2D.h>
#include <Clockwork/Clockwork2D/ConstraintGear2D.h>
#include <Clockwork/Clockwork2D/ConstraintMotor2D.h>
#include <Clockwork/Clockwork2D/ConstraintMouse2D.h>
#include <Clockwork/Clockwork2D/ConstraintPrismatic2D.h>
#include <Clockwork/Clockwork2D/ConstraintPulley2D.h>
#include <Clockwork/Clockwork2D/ConstraintRevolute2D.h>
#include <Clockwork/Clockwork2D/ConstraintRope2D.h>
#include <Clockwork/Clockwork2D/ConstraintWeld2D.h>
#include <Clockwork/Clockwork2D/ConstraintWheel2D.h>
...
#include <Clockwork/UI/Text3D.h>
#include <Clockwork/Container/Vector.h>
#include <Clockwork/Graphics/Zone.h>

#include "Clockwork2DConstraints.h"

DEFINE_APPLICATION_MAIN(Clockwork2DConstraints)

Node* pickedNode;
RigidBody2D* dummyBody;

Clockwork2DConstraints::Clockwork2DConstraints(Context* context) :
    Sample(context)
{
}

void Clockwork2DConstraints::Start()
{
...
void Clockwork2DConstraints::CreateFlag(const String& text, float x, float y) // Used to create Tex3D flags
{
...
void Clockwork2DConstraints::CreateInstructions()
{
    ResourceCache* cache = GetSubsystem<ResourceCache>();
...
void Clockwork2DConstraints::MoveCamera(float timeStep)
{
...
void Clockwork2DConstraints::SubscribeToEvents()
{
    // Subscribe HandleUpdate() function for processing update events
    SubscribeToEvent(E_UPDATE, HANDLER(Clockwork2DConstraints, HandleUpdate));

    // Subscribe HandlePostRenderUpdate() function for processing the post-render update event, during which we request debug geometry
    SubscribeToEvent(E_POSTRENDERUPDATE, HANDLER(Clockwork2DConstraints, HandlePostRenderUpdate));

    // Subscribe to mouse click
    SubscribeToEvent(E_MOUSEBUTTONDOWN, HANDLER(Clockwork2DConstraints, HandleMouseButtonDown));

    // Unsubscribe the SceneUpdate event from base class to prevent camera pitch and yaw in 2D sample
    UnsubscribeFromEvent(E_SCENEUPDATE);

    if (touchEnabled_)
        SubscribeToEvent(E_TOUCHBEGIN, HANDLER(Clockwork2DConstraints, HandleTouchBegin3));
}

void Clockwork2DConstraints::HandleUpdate(StringHash eventType, VariantMap& eventData)
{
...
void Clockwork2DConstraints::HandlePostRenderUpdate(StringHash eventType, VariantMap& eventData)
{
    PhysicsWorld2D* physicsWorld = scene_->GetComponent<PhysicsWorld2D>();
    if (drawDebug_) physicsWorld->DrawDebugGeometry();
}

void Clockwork2DConstraints::HandleMouseButtonDown(StringHash eventType, VariantMap& eventData)
{
    Input* input = GetSubsystem<Input>();
    PhysicsWorld2D* physicsWorld = scene_->GetComponent<PhysicsWorld2D>();
    ...
    SubscribeToEvent(E_MOUSEMOVE, HANDLER(Clockwork2DConstraints, HandleMouseMove));
    SubscribeToEvent(E_MOUSEBUTTONUP, HANDLER(Clockwork2DConstraints, HandleMouseButtonUp));
}
...
Vector2 Clockwork2DConstraints::GetMousePositionXY()
{
    Input* input = GetSubsystem<Input>();
    Graphics* graphics = GetSubsystem<Graphics>();
...
void Clockwork2DConstraints::HandleMouseMove(StringHash eventType, VariantMap& eventData)
{
    if (pickedNode)
    {
...
void Clockwork2DConstraints::HandleTouchBegin3(StringHash eventType, VariantMap& eventData)
{
    Graphics* graphics = GetSubsystem<Graphics>();
    PhysicsWorld2D* physicsWorld = scene_->GetComponent<PhysicsWorld2D>();
    ...
    SubscribeToEvent(E_TOUCHMOVE, HANDLER(Clockwork2DConstraints, HandleTouchMove3));
    SubscribeToEvent(E_TOUCHEND, HANDLER(Clockwork2DConstraints, HandleTouchEnd3));
}

void Clockwork2DConstraints::HandleTouchMove3(StringHash eventType, VariantMap& eventData)
{
    if (pickedNode)
    {
        Graphics* graphics = GetSubsystem<Graphics>();
...
void Clockwork2DConstraints::HandleTouchEnd3(StringHash eventType, VariantMap& eventData)
{
    if (pickedNode)
    {
        StaticSprite2D* staticSprite = pickedNode->GetComponent<StaticSprite2D>();
