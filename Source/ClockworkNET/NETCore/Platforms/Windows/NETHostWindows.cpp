...
bool NETHostWindows::CreateAppDomain()
{
    ...
    HRESULT hr  = clrRuntimeHost_->CreateAppDomainWithManager(
                W("ClockworkNETDomain"),   // The friendly name of the AppDomain
                // Flags:
...
bool NETHostWindows::TPAListContainsFile(wchar_t* fileNameWithoutExtension, wchar_t** rgTPCExtensions, int countExtensions)
{
    ...
    for (int iExtension = 0; iExtension < countExtensions; iExtension++)
    {
        ...
        wcscat_s(fileName, MAX_LONGPATH, rgTPCExtensions[iExtension] + 1);
...
void NETHostWindows::AddFilesFromDirectoryToTPAList(const wchar_t* targetPath, wchar_t** rgTPCExtensions, int countExtensions)
{
    ...
    for (int iExtension = 0; iExtension < countExtensions; iExtension++)
    {
        ...
        wcscat_s(assemblyPath, rgTPCExtensions[iExtension]);
        ...
        if (findHandle != INVALID_HANDLE_VALUE)
        {
            do
            {
                if (!(data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
                {
                    ...
                    if (!TPAListContainsFile(fileNameWithoutExtension, rgTPCExtensions, countExtensions))
                    {
...
bool NETHostWindows::GenerateTPAList()
{
    ...
    AddFilesFromDirectoryToTPAList(WString(coreCLRFilesAbsPath_).CString(), rgTPCExtensions, _countof(rgTPCExtensions));

    Vector<String> paths = coreCLRTPAPaths_.Split(';');
    for (unsigned i = 0; i < paths.Size(); i++)
    {
        AddFilesFromDirectoryToTPAList(WString(GetNativePath(paths[i])).CString(), rgTPCExtensions, _countof(rgTPCExtensions));
    }
