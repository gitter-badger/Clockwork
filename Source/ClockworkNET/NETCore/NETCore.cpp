...
CLOCKWORK_EXPORT_API ClassID csb_Clockwork_RefCounted_GetClassID(RefCounted* refCounted)
{
    if (!refCounted)
        return 0;
...
CLOCKWORK_EXPORT_API RefCounted* csb_ClockworkEngine_GetSubsystem(const char* name)
{
    return NETCore::GetContext()->GetSubsystem(name);
}

CLOCKWORK_EXPORT_API void csb_ClockworkEngine_ReleaseRef(RefCounted* ref)
{
    if (!ref)
        return;
...
void NETCore::AddAssemblyLoadPath(const String& assemblyPath)
{
    ...
    bool result = CreateDelegate(
                    "ClockworkNETBootstrap",
                    "Clockwork.Bootstrap.ClockworkLoadContext",
                    "AddAssemblyLoadPath",
