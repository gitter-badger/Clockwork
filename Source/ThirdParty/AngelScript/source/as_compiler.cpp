...
int asCCompiler::DoAssignment(asSExprContext *ctx, asSExprContext *lctx, asSExprContext *rctx, asCScriptNode *lexpr, asCScriptNode *rexpr, int op, asCScriptNode *opNode)
{
	...
	// Clockwork: if there is a handle type, and it does not have an overloaded assignment operator, convert to an explicit handle
	// for scripting convenience. (For the Clockwork handle types, value assignment is not supported)
