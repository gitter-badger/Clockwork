 ...
 (lua_State* L, int lo, int dim, int def, tolua_Error* err)
{
	...
	{
		int i;
		if (dim == -1)		// Clockwork - auto detect the array size if -1 is given
 ...
 (lua_State* L, int lo, const char* type, int dim, int def, tolua_Error* err)
{
	...
	else
	{
		...
		for (i=1; i<=dim; ++i)
		{
			lua_pushnumber(L,i);
			lua_gettable(L,lo);
	  if (!(lua_isnil(L,-1) || lua_isusertype(L,-1, type)) &&	// Clockwork - bug fix to check user type instead of user data
			    !(def && lua_isnil(L,-1))
						)
			{
				err->index = lo;
