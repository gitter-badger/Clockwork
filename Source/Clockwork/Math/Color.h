...
public:
    ...
    Color Abs() const { return Color(Clockwork::Abs(r_), Clockwork::Abs(g_), Clockwork::Abs(b_), Clockwork::Abs(a_)); }
    /// Test for equality with another color with epsilon.
    bool Equals(const Color& rhs) const { return Clockwork::Equals(r_, rhs.r_) && Clockwork::Equals(g_, rhs.g_) && Clockwork::Equals(b_, rhs.b_) && Clockwork::Equals(a_, rhs.a_); }
