...
bool XMLFile::CombineText(const pugi::xml_node& patch, pugi::xml_node original, bool prepend)
{
    ...
    {
        if (prepend)
            original.set_value(Clockwork::ToString("%s%s", patch.value(), original.value()).CString());
        else
            original.set_value(Clockwork::ToString("%s%s", original.value(), patch.value()).CString());
