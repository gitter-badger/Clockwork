...
var cli = require("clockwork-cli");

var ClockworkHelpFormatter = function (opts) {
    argparse.HelpFormatter.call(this, opts);
};
util.inherits(ClockworkHelpFormatter, argparse.HelpFormatter);

// http://stackoverflow.com/questions/13423540/argparse-subparser-hide-metavar-in-command-listing
ClockworkHelpFormatter.prototype._formatAction = function (action) {
...
var parser = new argparse.ArgumentParser({prog: "clockwork-cli", formatterClass: ClockworkHelpFormatter,
    description: "Clockwork Game Engine CLI"});
...
cmd.setDefaults({action: function (args) {
    ...
    .then(function () {
        console.log("New Clockwork project created in " + path.resolve(args.path));
    })
...
var cmd = commands.addParser("editor", {help: "Starts the Clockwork Editor and loads current project",
    description: "Starts the Clockwork Editor and loads current project.",
    aliases: ["edit"]});
cmd.setDefaults({action: function (args) {
            cli.editor()
}});
