...
var saveClockworkConfig = function(activated) {

  var directory = osenv.home() + "/.clockworkgameengine";
    ...
    prompt.get([ { description: 'Please confirm EULA agreement (Y)es, (N)o, or (V)iew', default: "Y", name: "eulaconfirm" }], function (err, result) {
        ...
        if (eulaconfirm == 'y') {
          prompt.get([ { description: 'Please enter Product Key or (G)et free key: ', name: "productkey" }], function (err, result) {
            ...
            if (productkey == 'g') {
              console.log ("Opening Clockwork Store in default browser window");
              open("https://store.clockworkgameengine.com/site");
            } else {
              cli.activate(productkey)
              .then(function () {
                saveClockworkConfig(true);
              })
            }
          });
        } else if (eulaconfirm == 'v') {
          console.log ("Opening EULA in default browser window");
          open("https://github.com/dragonCASTjosh/Clockwork/blob/master/LICENSE.md");
        }
    });
  });
...
program
  ...
  .action(function(folder){
    ...
    .then(function () {
        console.log("New Clockwork project created in " + path.resolve(folder));
    })
...
program
  ...
  .action(function(path_to_project, options){
    ...
    cli.clockworkeditor(["-project", path], {output:true});
  });
