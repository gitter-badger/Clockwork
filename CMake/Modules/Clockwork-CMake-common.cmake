...
if (CMAKE_CONFIGURATION_TYPES)
    ...
    set (CMAKE_CONFIGURATION_TYPES ${CLOCKWORK_BUILD_CONFIGURATIONS} CACHE STRING "${DOC_STRING}" FORCE)
...
if (NOT MSVC AND NOT DEFINED CLOCKWORK_DEFAULT_64BIT)  # Only do this once in the initial configure step
    ...
    if (matched)
        set (CLOCKWORK_DEFAULT_64BIT TRUE)
    else ()
        set (CLOCKWORK_DEFAULT_64BIT FALSE)
    endif ()
    set (CLOCKWORK_DEFAULT_64BIT ${CLOCKWORK_DEFAULT_64BIT} CACHE INTERNAL "Default value for CLOCKWORK_64BIT build option")
    # The 'ANDROID' CMake variable is already set by android.toolchain.cmake when it is being used for cross-compiling Android
    # The other arm platform that Clockwork supports that is not Android is Raspberry Pi at the moment
...
if (ANDROID OR RPI OR EMSCRIPTEN)
    ...
    set (CLOCKWORK_64BIT ${CLOCKWORK_DEFAULT_64BIT})
else ()
...
option (CLOCKWORK_LUA "Enable additional Lua scripting support")
...
if (MINGW AND NOT DEFINED CLOCKWORK_SSE)
    ...
    if (GCC_VERSION VERSION_LESS 4.9.1)
        message (WARNING "Disabling SSE by default due to MinGW version. It is recommended to upgrade to MinGW with GCC >= 4.9.1. You can also try to re-enable SSE with CMake option -DCLOCKWORK_SSE=1, but this may result in compile errors.")
        set (CLOCKWORK_DEFAULT_SSE FALSE)
    else ()
        set (CLOCKWORK_DEFAULT_SSE TRUE)
    endif ()
else ()
    set (CLOCKWORK_DEFAULT_SSE TRUE)
endif ()
cmake_dependent_option (CLOCKWORK_SSE "Enable SSE instruction set" ${CLOCKWORK_DEFAULT_SSE} "NOT EMSCRIPTEN" FALSE)
if (CMAKE_PROJECT_NAME STREQUAL Clockwork)
    ...
    if (CPACK_SYSTEM_NAME STREQUAL Linux)
        cmake_dependent_option (CLOCKWORK_USE_LIB64_RPM "Enable 64-bit RPM CPack generator using /usr/lib64 and disable all other generators (Debian-based host only)" FALSE "CLOCKWORK_64BIT AND NOT HAS_LIB64" FALSE)
        cmake_dependent_option (CLOCKWORK_USE_LIB_DEB "Enable 64-bit DEB CPack generator using /usr/lib and disable all other generators (Redhat-based host only)" FALSE "CLOCKWORK_64BIT AND HAS_LIB64" FALSE)
    endif ()
else ()
    set (CLOCKWORK_HOME "" CACHE PATH "Path to Clockwork build tree or SDK installation location (external project only)")
endif ()
option (CLOCKWORK_PACKAGING "Enable resources packaging support, on Emscripten default to 1, on other platforms default to 0" ${EMSCRIPTEN})
option (CLOCKWORK_PROFILING "Enable profiling support" TRUE)
option (CLOCKWORK_LOGGING "Enable logging support" TRUE)
option (CLOCKWORK_TESTING "Enable testing support")
if (CLOCKWORK_TESTING)
    if (EMSCRIPTEN)
    ...
    set (CLOCKWORK_TEST_TIMEOUT ${DEFAULT_TIMEOUT} CACHE STRING "Number of seconds to test run the executables (when testing support is enabled only), default to 10 on Emscripten platform and 5 on other platforms")
else ()
    unset (CLOCKWORK_TEST_TIMEOUT CACHE)
    if (EMSCRIPTEN_EMRUN_BROWSER)   # Suppress unused variable warning at the same time
        unset (EMSCRIPTEN_EMRUN_BROWSER CACHE)
    endif ()
endif ()
# The CLOCKWORK_OPENGL option is not defined on non-Windows platforms as they should always use OpenGL
if (MSVC)
    # On MSVC compiler, default to false (i.e. prefers Direct3D)
    # OpenGL can be manually enabled with -DCLOCKWORK_OPENGL=1, but Windows graphics card drivers are usually better optimized for Direct3D
    ...
    # Direct3D can be manually enabled with -DCLOCKWORK_OPENGL=0, but it is likely to fail unless the MinGW-w64 distribution is used due to dependency to Direct3D headers and libs
    set (DEFAULT_OPENGL TRUE)
endif ()
cmake_dependent_option (CLOCKWORK_OPENGL "Use OpenGL instead of Direct3D (Windows platform only)" ${DEFAULT_OPENGL} "WIN32" TRUE)      # Force the variable to TRUE when not WIN32
...
cmake_dependent_option (CLOCKWORK_D3D11 "Use Direct3D11 instead of Direct3D9 (Windows platform only); overrides CLOCKWORK_OPENGL option" FALSE "WIN32" FALSE)
if (CMAKE_HOST_WIN32 AND NOT DEFINED CLOCKWORK_MKLINK)
    ...
    if (MKLINK_EXIT_CODE EQUAL 0)
        set (CLOCKWORK_MKLINK TRUE)
        file (REMOVE ${CMAKE_BINARY_DIR}/test-link)
    else ()
        set (CLOCKWORK_MKLINK FALSE)
    endif ()
    set (CLOCKWORK_MKLINK ${CLOCKWORK_MKLINK} CACHE INTERNAL "MKLINK capability on the Windows host system")
endif ()
cmake_dependent_option (CLOCKWORK_STATIC_RUNTIME "Use static C/C++ runtime libraries and eliminate the need for runtime DLLs installation (VS only)" FALSE "MSVC" FALSE)
cmake_dependent_option (CLOCKWORK_WIN32_CONSOLE "Use console main() as entry point when setting up Windows executable targets (Windows platform only)" FALSE "WIN32" FALSE)
cmake_dependent_option (CLOCKWORK_MACOSX_BUNDLE "Use MACOSX_BUNDLE when setting up Mac OS X executable targets (Xcode native build only)" FALSE "XCODE AND NOT IOS" FALSE)
set (CLOCKWORK_LIB_TYPE STATIC CACHE STRING "Specify Clockwork library type, possible values are STATIC (default) and SHARED")
if (CMAKE_CROSSCOMPILING AND NOT ANDROID)
    set (CLOCKWORK_SCP_TO_TARGET "" CACHE STRING "Use scp to transfer executables to target system (non-Android cross-compiling build only), SSH digital key must be setup first for this to work, typical value has a pattern of usr@tgt:remote-loc")
else ()
    unset (CLOCKWORK_SCP_TO_TARGET CACHE)
endif ()
if (ANDROID)
...
if (CMAKE_VERSION VERSION_GREATER 2.8 OR CMAKE_VERSION VERSION_EQUAL 2.8)
    set_property (CACHE CLOCKWORK_LIB_TYPE PROPERTY STRINGS STATIC SHARED)
    if (NOT CMAKE_CONFIGURATION_TYPES)
        set_property (CACHE CMAKE_BUILD_TYPE PROPERTY STRINGS ${CLOCKWORK_BUILD_CONFIGURATIONS})
    endif ()
    if (RPI)
...
if (MSVC AND CLOCKWORK_MINIDUMPS)
    add_definitions (-DCLOCKWORK_MINIDUMPS)
endif ()

# By default use the MSVC dynamic runtime. To eliminate the need to distribute the runtime installer,
# this can be switched off if not using Clockwork as a shared library.
if (MSVC)
    if (CLOCKWORK_STATIC_RUNTIME)
...
if (CLOCKWORK_D3D11)
    set (CLOCKWORK_OPENGL 0)
    add_definitions (-DCLOCKWORK_D3D11)
endif ()

# Add definition for OpenGL
if (CLOCKWORK_OPENGL)
    add_definitions (-DCLOCKWORK_OPENGL)
endif ()

# Add definitions for GLEW
if (NOT IOS AND NOT ANDROID AND NOT RPI AND CLOCKWORK_OPENGL)
...
if (CLOCKWORK_LUAJIT)
    add_definitions (-DCLOCKWORK_LUAJIT)
    set (JIT JIT)
    # Implied CLOCKWORK_LUA
    set (CLOCKWORK_LUA 1)
endif ()
if (CLOCKWORK_LUA)
    add_definitions (-DCLOCKWORK_LUA)
    # Optionally enable Lua / C++ wrapper safety checks
    if (NOT CLOCKWORK_SAFE_LUA)
...
if (CLOCKWORK_NAVIGATION)
    add_definitions (-DCLOCKWORK_NAVIGATION)
endif ()

# Add definition for Network
if (CLOCKWORK_NETWORK)
    add_definitions (-DCLOCKWORK_NETWORK)
endif ()

# Add definition for Physics
if (CLOCKWORK_PHYSICS)
    add_definitions (-DCLOCKWORK_PHYSICS)
endif ()

# Add definition for Clockwork2D
if (CLOCKWORK_CLOCKWORK2D)
...
if (MSVC)
    ...
    if (CLOCKWORK_SSE AND NOT CLOCKWORK_64BIT)
...
macro (check_source_files)
    if (NOT SOURCE_FILES)
        message (FATAL_ERROR "Could not configure and generate the project file because no source files have been defined yet. "
            ...
            "See the define_source_files() macro definition in the CMake/Modules/Clockwork-CMake-common.cmake for more detail.")
    endif ()
endmacro ()
...
macro (setup_library)
    ...
    elseif (CLOCKWORK_SCP_TO_TARGET)
...
# Macro arguments:
#  NODEPS - setup executable target without defining Clockwork dependency libraries
...
macro (setup_executable)
    ...
    if (ARG_NODEPS)
        define_dependency_libs (Clockwork-nodeps)
    else ()
        define_dependency_libs (Clockwork)
    endif ()
    setup_target ()

    if (CLOCKWORK_SCP_TO_TARGET)
        add_custom_command (TARGET ${TARGET_NAME} POST_BUILD COMMAND scp $<TARGET_FILE:${TARGET_NAME}> ${CLOCKWORK_SCP_TO_TARGET} || exit 0
...
# Macro for finding file in Clockwork build tree or Clockwork SDK
macro (find_Clockwork_file VAR NAME)
    # Parse extra arguments
    ...
    if (NOT ${VAR} AND ARG_MSG_MODE)
        message (${ARG_MSG_MODE}
            "Could not find ${VAR} file in the Clockwork build tree or Clockwork SDK. "
            "Please reconfigure and rebuild your Clockwork build tree or reinstall the SDK for the correct target platform.")
    endif ()
endmacro ()

# Macro for finding tool in Clockwork build tree or Clockwork SDK
macro (find_Clockwork_tool VAR NAME)
    ...
    if (NOT ${VAR})
        ...
        if (ARG_MSG_MODE AND NOT CMAKE_PROJECT_NAME STREQUAL Clockwork)
            message (${ARG_MSG_MODE}
                "Could not find ${VAR} tool in the Clockwork build tree or Clockwork SDK. Your project may not build successfully without this tool. "
                "You may have to first rebuild the Clockwork in its build tree or reinstall Clockwork SDK to get this tool built or installed properly. "
...
# Macro arguments:
#  NODEPS - setup executable target without defining Clockwork dependency libraries
#  NOBUNDLE - do not use MACOSX_BUNDLE even when CLOCKWORK_MACOSX_BUNDLE build option is enabled
#  WIN32/MACOSX_BUNDLE/EXCLUDE_FROM_ALL - see CMake help on add_executable() command
# CMake variables:
#  RESOURCE_DIRS - list of resource directories (will be packaged into *.pak when CLOCKWORK_PACKAGING build option is set)
...
macro (setup_main_executable)
    ...
    if (NOT RESOURCE_DIRS)
        # If the macro caller has not defined the resource dirs then set them based on Clockwork project convention
        foreach (DIR ${CMAKE_SOURCE_DIR}/bin/CoreData ${CMAKE_SOURCE_DIR}/bin/Data)
            # Do not assume external project always follows Clockwork project convention, so double check if this directory exists before using it
            if (IS_DIRECTORY ${DIR})
                list (APPEND RESOURCE_DIRS ${DIR})
            endif ()
        endforeach ()
    endif ()
    if (CLOCKWORK_PACKAGING AND RESOURCE_DIRS)
        ...
        if (EMSCRIPTEN)
            ...
            if (NOT SHELL_HTML_FOUND)
                # Use custom Clockwork shell.html
    ...
    if (XCODE)
        if (NOT RESOURCE_FILES)
            ...
            set (RESOURCE_FILES ${CMAKE_SOURCE_DIR}/bin/Data/Textures/ClockworkIcon.icns)
            if (IOS)
                # Default app icon on the iOS home screen
                list (APPEND RESOURCE_FILES ${CMAKE_SOURCE_DIR}/bin/Data/Textures/ClockworkIcon.png)
            endif ()
        endif ()
    ...
    if (ANDROID)
        ...
        find_Clockwork_file (ANDROID_MAIN_C_PATH SDL_android_main.c
            HINTS ${CLOCKWORK_HOME}/include/${PATH_SUFFIX}/ThirdParty/SDL/android ${CMAKE_SOURCE_DIR}/Source/ThirdParty/SDL/src/main/android
        ...
        if (CMAKE_PROJECT_NAME STREQUAL Clockwork AND NOT APK_INCLUDED)
            install (FILES ${LIBRARY_OUTPUT_PATH_ROOT}/bin/Clockwork-debug.apk DESTINATION ${DEST_RUNTIME_DIR} OPTIONAL)
            set (APK_INCLUDED 1)
        endif ()
    else ()
        # Setup target as executable
        if (WIN32)
            if (NOT CLOCKWORK_WIN32_CONSOLE OR ARG_WIN32)
                set (EXE_TYPE WIN32)
            endif ()
        ...
        elseif (APPLE)
            if ((CLOCKWORK_MACOSX_BUNDLE OR ARG_MACOSX_BUNDLE) AND NOT ARG_NOBUNDLE)
                set (EXE_TYPE MACOSX_BUNDLE)
    ...
    if ((EXE_TYPE STREQUAL MACOSX_BUNDLE OR CLOCKWORK_PACKAGING) AND RESOURCE_DIRS)
...
macro (setup_test)
    if (CLOCKWORK_TESTING)
        ...
        list (APPEND ARG_OPTIONS -timeout ${CLOCKWORK_TEST_TIMEOUT})
        if (EMSCRIPTEN)
            if (DEFINED ENV{CI})
                ...
                set (EMRUN_TIMEOUT_ADJUSTMENT + 8 * \\${CLOCKWORK_TEST_TIMEOUT})
                set (EMRUN_TIMEOUT_RETURNCODE --timeout_returncode 0)
            endif ()
            math (EXPR EMRUN_TIMEOUT "2 * ${CLOCKWORK_TEST_TIMEOUT} ${EMRUN_TIMEOUT_ADJUSTMENT}")
...
# It works for both targets setup within Clockwork project and outside Clockwork project that uses Clockwork as external static/shared library
macro (define_dependency_libs TARGET)
    # ThirdParty/SDL external dependency
    if (${TARGET} MATCHES SDL|Clockwork)
        if (WIN32)
    ...
    # Clockwork/LuaJIT external dependency
    if (CLOCKWORK_LUAJIT AND ${TARGET} MATCHES LuaJIT|Clockwork)
        if (NOT WIN32 AND NOT EMSCRIPTEN)
            list (APPEND LIBS dl m)
        endif ()
    endif ()

    # Clockwork external dependency
    if (${TARGET} STREQUAL Clockwork)
        # Core
        if (WIN32)
        ...
        if (CLOCKWORK_LIBRARIES)
            if (WIN32 AND CLOCKWORK_LIBRARIES_DBG AND CLOCKWORK_LIBRARIES_REL AND TARGET ${TARGET_NAME})
                # Special handling when both debug and release libraries are found
                target_link_libraries (${TARGET_NAME} debug ${CLOCKWORK_LIBRARIES_DBG} optimized ${CLOCKWORK_LIBRARIES_REL})
            else ()
                list (APPEND ABSOLUTE_PATH_LIBS ${CLOCKWORK_LIBRARIES})
            endif ()
        endif ()
    endif ()

    # LuaJIT specific - extra linker flags for linking against LuaJIT (adapted from LuaJIT's original Makefile)
    if (CLOCKWORK_LUAJIT AND ${TARGET} MATCHES Clockwork)
        # 64-bit Mac OS X
        if (CLOCKWORK_64BIT AND APPLE AND NOT IOS)
...
if (ANDROID)
    ...
    endforeach ()
elseif (EMSCRIPTEN)
    # Create Clockwork custom HTML shell that also embeds our own project logo
