...
desc 'Create a new project using Clockwork as external library'
task :scaffolding do
...
desc 'Configure, build, and test Clockwork project'
task :ci do
  ...
  if ENV['PACKAGE_UPLOAD']
    ...
    $testing = (ENV['LINUX'] && !ENV['CLOCKWORK_64BIT']) || (ENV['OSX'] && ENV['IOS'].to_i != 1) || ENV['EMSCRIPTEN'] ? 1 : 0
    if $testing
      ENV['CLOCKWORK_PREFIX_PATH'] = `pwd`.chomp + '/bin'
    end
  end
  ...
  ['CLOCKWORK_64BIT', 'CLOCKWORK_LIB_TYPE', 'CLOCKWORK_OPENGL', 'CLOCKWORK_D3D11', 'CLOCKWORK_TEST_TIMEOUT', 'ANDROID', 'RPI', 'RPI_ABI', 'EMSCRIPTEN', 'EMSCRIPTEN_SHARE_DATA', 'EMSCRIPTEN_EMRUN_BROWSER'].each { |var| $build_options = "#{$build_options} -D#{var}=#{ENV[var]}" if ENV[var] }
  if ENV['XCODE']
    # xcodebuild
    xcode_ci
  else
...
task :ci_site_update do
  ...
  unless ENV['RELEASE_TAG'] || `git fetch -qf origin #{ENV['TRAVIS_BRANCH']}; git log -1 --pretty=format:'%H' FETCH_HEAD` != ENV['TRAVIS_COMMIT']
    ...
    if system("git commit -qm 'Test commit to detect API changes'")
      ...
      bump_soversion 'Source/Clockwork/.soversion' or abort 'Failed to bump soversion'
      system "git add Source/Clockwork/.soversion && git commit --amend -qm \"Travis CI: API documentation update at #{Time.now.utc}.\n#{ENV['PACKAGE_UPLOAD'] ? '' : '[ci package]'}\n\nCommit: https://github.com/$TRAVIS_REPO_SLUG/commit/$TRAVIS_COMMIT\n\nMessage: $COMMIT_MESSAGE\"" or abort 'Failed to stage .soversion file'
...
task :ci_package_upload do
  ...
  if ENV['SITE_UPDATE']
    ...
    if ENV['XCODE']
      xcode_build(ENV['IOS'], '../Build/Clockwork.xcodeproj', 'doc', '>/dev/null') or abort 'Failed to generate documentation'
    else
  ...
  if ENV['IOS']
    ...
    if !ENV['CI_START_TIME'] || elapsed_time < 25 # minutes
      ...
      system 'echo Rebuilding Clockwork library as Mach-O universal binary...'
      xcode_build(0, '../Build/Clockwork.xcodeproj', 'Clockwork_universal') or abort 'Failed to build Mach-O universal binary'
    end
    ...
    xcode_build(ENV['IOS'], '../Build/Clockwork.xcodeproj', 'package') or abort 'Failed to make binary package'
  else
    if ENV['ANDROID'] && !ENV['NO_SDK_SYSIMG']
      system "cd ../Build && android update project -p . -t $(android list target |grep android-$API |cut -d ' ' -f2) && ant debug" or abort 'Failed to make Clockwork Samples APK'
    end
    if ENV['CLOCKWORK_USE_LIB64_RPM']
      system "cd ../Build && cmake . -DCLOCKWORK_USE_LIB64_RPM=#{ENV['CLOCKWORK_USE_LIB64_RPM']}" or abort 'Failed to reconfigure to generate 64-bit RPM package'
    end
    wrapper = ENV['CLOCKWORK_64BIT'] || ENV['RPI'] ? 'setarch i686' : ''
  ...
  unless ENV['RELEASE_TAG']
    ...
    if ENV['SITE_UPDATE']
      ...
      system "export SNAPSHOT_VER=$(git describe $TRAVIS_COMMIT |ruby -pe 'gsub(/-(?!g)/, %q{.})'); wget -q https://github.com/$TRAVIS_REPO_SLUG/tarball/$TRAVIS_COMMIT -O Clockwork-$SNAPSHOT_VER-Source-snapshot.tar.gz && wget -q https://github.com/$TRAVIS_REPO_SLUG/zipball/$TRAVIS_COMMIT -O Clockwork-$SNAPSHOT_VER-Source-snapshot.zip" or abort 'Failed to get source packages'
      # Only keep the snapshots from the last 30 revisions
      system "for v in $(sftp clockwork-travis-ci@frs.sourceforge.net <<EOF |tr ' ' '\n' |grep Clockwork- |cut -d '-' -f1,2 |uniq |tail -n +31
cd #{upload_dir}
ls -1r
bye
EOF
); do echo rm #{upload_dir}/${v}*; done |sftp -b - clockwork-travis-ci@frs.sourceforge.net >/dev/null 2>&1" or abort 'Failed to housekeep snapshots'
    end
  else
    upload_dir = "/home/frs/project/#{ENV['TRAVIS_REPO_SLUG']}/#{ENV['RELEASE_TAG']}"
    if ENV['SITE_UPDATE']
      # Download source packages from GitHub
      system 'wget -q https://github.com/$TRAVIS_REPO_SLUG/archive/$RELEASE_TAG.tar.gz -O Clockwork-$RELEASE_TAG-Source.tar.gz && wget -q https://github.com/$TRAVIS_REPO_SLUG/archive/$RELEASE_TAG.zip -O Clockwork-$RELEASE_TAG-Source.zip' or abort 'Failed to get source packages'
    end
    ...
    system "sftp clockwork-travis-ci@frs.sourceforge.net <<EOF >/dev/null 2>&1
mkdir #{upload_dir}
bye
EOF" or abort 'Failed to create release directory remotely'
  ...
  if ENV['SITE_UPDATE']
    ...
    system "scp Clockwork-* clockwork-travis-ci@frs.sourceforge.net:#{upload_dir}" or abort 'Failed to upload source package'
    if ENV['RELEASE_TAG']
      # Mark the source tarball as default download for host systems other than Windows/Mac/Linux
      system "curl -H 'Accept: application/json' -X PUT -d 'default=bsd&default=solaris&default=others' -d \"api_key=$SF_API\" https://sourceforge.net/projects/%s/files/%s/#{ENV['RELEASE_TAG']}/Clockwork-#{ENV['RELEASE_TAG']}-Source.tar.gz" % ENV['TRAVIS_REPO_SLUG'].split('/') or abort 'Failed to set source tarball as default download'
    end
    ...
    system 'rsync -e ssh -az README.md License.txt clockwork-travis-ci@frs.sourceforge.net:/home/frs/project/$TRAVIS_REPO_SLUG' or abort 'Failed to sync readme and license files'
    # Mark that the site has been updated
    File.open('.site_updated', 'w') {}
  end
  # Upload the binary package
  system "scp ../Build/Clockwork-* clockwork-travis-ci@frs.sourceforge.net:#{upload_dir} && rm ../Build/Clockwork-*" or abort 'Failed to upload binary package'
  if ENV['RELEASE_TAG'] && ENV['SF_DEFAULT']
    ...
    system "curl -H 'Accept: application/json' -X PUT -d 'default=%s' -d \"api_key=$SF_API\" https://sourceforge.net/projects/%s/files/%s/#{ENV['RELEASE_TAG']}/Clockwork-#{ENV['RELEASE_TAG']}-%s" % ENV['SF_DEFAULT'].split(':').insert(1, ENV['TRAVIS_REPO_SLUG'].split('/')).flatten or abort 'Failed to set binary tarball/zip as default download'
  end
end
...
EOF
  ...
  if ENV['OS']
    system("@echo off && mkdir '#{dir}'\\bin && copy Source\\Tools\\ClockworkPlayer\\ClockworkPlayer.* '#{dir}' >nul && (for %f in (*.bat Rakefile) do mklink '#{dir}'\\%f %cd%\\%f >nul) && mklink /D '#{dir}'\\CMake %cd%\\CMake && (for %d in (CoreData,Data) do mklink /D '#{dir}'\\bin\\%d %cd%\\bin\\%d >nul)") && File.write("#{dir}/CMakeLists.txt", build_script) or abort 'Failed to create new project using Clockwork as external library'
  else
    system("bash -c \"mkdir -p '#{dir}'/bin && cp Source/Tools/ClockworkPlayer/ClockworkPlayer.* '#{dir}' && for f in {.,}*.sh Rakefile CMake; do ln -sf `pwd`/\\$f '#{dir}'; done && ln -sf `pwd`/bin/{Core,}Data '#{dir}'/bin\"") && File.write("#{dir}/CMakeLists.txt", build_script) or abort 'Failed to create new project using Clockwork as external library'
  end
end
...
def makefile_ci
  ...
  system "cd ../Build && make -j$NUMJOBS #{test}" or abort 'Failed to build or test Clockwork library'
  ...
  if ENV['AVD'] && !ENV['PACKAGE_UPLOAD']
    system "echo '\nTest deploying and running Clockwork Samples APK...' && cd ../Build && android update project -p . -t $( android list target |grep android-$API |cut -d ' ' -f2 ) && ant debug" or abort 'Failed to make Clockwork Samples APK'
    if android_wait_for_device
      system "cd ../Build && ant -Dadb.device.arg='-s #{$specific_device}' installd" or abort 'Failed to deploy Clockwork Samples APK'
      android_test_run or abort 'Failed to test run Clockwork Samples APK'
    else
      puts 'Skipped test running Clockwork Samples APK as emulator failed to start in time'
    end
  end
end
