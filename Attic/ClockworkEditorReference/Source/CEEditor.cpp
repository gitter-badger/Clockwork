...
Editor::Editor(Context* context) :
    Object(context),
    currentPlatform_(CE_PLATFORM_UNDEFINED),
    requestExit_(false)

{
    RegisterEditorLibrary(context_);

    aejavascript_ = new CEJavascript(context_);
    aepreferences_ = new CEPreferences(context_);
...
CEPreferences* Editor::GetPreferences()
{
...
{    
    ...
    if (player_->GetMode() != CE_PLAYERMODE_WIDGET)
    {
...
void Editor::RequestPlatformChange(CEEditorPlatform platform)
{
// BEGIN LICENSE MANAGEMENT
...
void Editor::HandlePlayRequest(StringHash eventType, VariantMap& eventData)
{
    ...
    CEPlayerMode mode = (CEPlayerMode) eventData[EditorPlayStarted::P_MODE].GetUInt();

    if (mode != CE_PLAYERMODE_WIDGET)
    {
        //tbui->SetInputDisabled(true);
    }

    player_ = new CEPlayer(context_);
