...
void LicenseSystem::Initialize()
{
    ...
    String eulaConfirmedFilePath = filesystem->GetAppPreferencesDir("ClockworkEditor", "License");
...
bool LicenseSystem::LoadLicense()
{
    ...
    FileSystem* filesystem = GetSubsystem<FileSystem>();
    String licenseFilePath = filesystem->GetAppPreferencesDir("ClockworkEditor", "License");
    licenseFilePath = AddTrailingSlash(licenseFilePath);
    licenseFilePath += "ClockworkLicense";
...
bool LicenseSystem::RequestPlatformChange(CEEditorPlatform platform)
{

    if (platform == CE_PLATFORM_HTML5 && LicenseHTML5())
        return true;

    if (platform == CE_PLATFORM_IOS && LicenseIOS())
        return true;

    if (platform == CE_PLATFORM_WINDOWS && LicenseWindows())
        return true;

    if (platform == CE_PLATFORM_MAC && LicenseMac())
        return true;

    if (platform == CE_PLATFORM_ANDROID && LicenseAndroid())
        return true;
