...
WelcomeFrame::WelcomeFrame(Context* context) :
    CEWidget(context), currentExampleLayout_(0), exampleCount_(0)
{
    ...
    tbui->LoadResourceFile(delegate_, "ClockworkEditor/editor/ui/welcomeframe.tb.txt");
...
void WelcomeFrame::UpdateRecentProjects()
{
    ...
    const Vector<String>& recent = GetSubsystem<CEPreferences>()->GetRecentProjects();
...
bool WelcomeFrame::OnEvent(const TBWidgetEvent &ev)
{
    ...
    if (ev.type == EVENT_TYPE_CLICK)
    {
        if (ev.target)
        {
            ...
            {
                fileSystem->SystemOpen("http://clockworkgameengine.com/videos");
            }
            else if (ev.target->GetID() ==  TBIDC("forum"))
            {
                fileSystem->SystemOpen("http://clockworkgameengine.com/forum");
            }
            else if (ev.target->GetID() == TBIDC("clockwork_github"))
            {
            ...
            {
                fileSystem->SystemOpen("https://github.com/ClockworkGameEngine/ClockworkExamples");
            }
