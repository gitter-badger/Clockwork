...
#include "License/CEVersionCheck.h"

#include "UIFindTextWidget.h"
#include "CEEvents.h"
#include "CEEditor.h"
#include "CEEditorStrings.h"
...
#include "Tools/External/CEExternalTooling.h"
    ...
    platformIndicator_(0)
{
    ...
    tbui->LoadResourceFile(delegate_, "ClockworkEditor/editor/ui/mainframe.tb.txt");
    ...
    consoletext_->SetText("Clockwork Editor Initialized");
...
void MainFrame::InitializeMenuSources()
{
    ...
    menuClockworkEditorSource.AddItem(new MenubarItem("About Clockwork Editor", TBIDC("about clockwork editor")));
    menuClockworkEditorSource.AddItem(new MenubarItem("-"));
    menuClockworkEditorSource.AddItem(new MenubarItem("Manage License", TBIDC("manage license")));
    menuClockworkEditorSource.AddItem(new MenubarItem("-"));
    menuClockworkEditorSource.AddItem(new MenubarItem("Check for Updates", TBIDC("check update")));
    menuClockworkEditorSource.AddItem(new MenubarItem("-"));
    menuClockworkEditorSource.AddItem(new MenubarItem("Quit", TBIDC("quit")));
    ...
    menuHelpSource.AddItem(new MenubarItem("-"));
    menuHelpSource.AddItem(new MenubarItem("Clockwork Game Engine on GitHub", TBIDC("help_github")));
...
bool MainFrame::UpdateJavascriptErrors()
{
    ...
    CEPlayer* player = GetSubsystem<CEPlayer>();
    if (player)
    {
        const Vector<CEPlayerError>& playererrors = player->GetErrors();
...
bool MainFrame::HandleMenubarEvent(const TBWidgetEvent &ev)
{
    ...
    {
        if (ev.target->GetID() == TBIDC("menu clockwork editor"))
        {
            if (TBMenuWindow *menu = new TBMenuWindow(ev.target, TBIDC("clockwork editor popup")))
            {
                menu->Show(&menuClockworkEditorSource, TBPopupAlignment());
            }
...
bool MainFrame::HandlePopupMenuEvent(const TBWidgetEvent &ev)
{
    ...
    if (ev.type == EVENT_TYPE_CLICK)
    {
        ...
        if (ev.target->GetID() == TBIDC("clockwork editor popup"))
        {
            if (ev.ref_id == TBIDC("about clockwork editor"))
            {
        ...
        if (ev.target->GetID() == TBIDC("tools popup"))
        {
            if (ev.ref_id == TBIDC("tools tiled"))
            {
                ...
                tooling->LaunchOrOpen("ClockworkTiled", "");
            }
        ...
        if (ev.target->GetID() == TBIDC("help popup"))
        {
            ...
            if (ev.ref_id == TBIDC("help_forums"))
            {
                ...
                fileSystem->SystemOpen("http://www.clockworkgameengine.com/forum");

            }
            if (ev.ref_id == TBIDC("help_github"))
            {

                fileSystem->SystemOpen("https://github.com/ClockworkGameEngine/ClockworkGameEngine");
...
void MainFrame::HandlePlatformChange(StringHash eventType, VariantMap& eventData)
{
    ...
    CEEditorPlatform platform = (CEEditorPlatform) eventData[P_PLATFORM].GetUInt();
