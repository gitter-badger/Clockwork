    ...
    if (eventType == E_SUBPROCESSCOMPLETE)
    {
        ...
        if (!code || currentBuildPhase_ == ADBListDevices)
        {
            ...
            else if (currentBuildPhase_ == ADBListDevices)
            {
                ...
                {
                    // can't proceed
                    buildSystem->BuildComplete(CE_PLATFORM_ANDROID, buildPath_);
                }
        ...
        else
        {
            ...
            buildSystem->BuildComplete(CE_PLATFORM_ANDROID, buildPath_, false);
        }
...
{
    Editor* editor = GetSubsystem<Editor>();
    CEPreferences* prefs = editor->GetPreferences();
    SubprocessSystem* subs = GetSubsystem<SubprocessSystem>();
