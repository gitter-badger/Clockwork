...
void BuildIOS::HandleEvent(StringHash eventType, VariantMap& eventData)
{
    ...
    if (eventType == E_SUBPROCESSCOMPLETE)
    {
        ...
        if (!code)
        {
            ...
            {
                buildSystem->BuildComplete(CE_PLATFORM_IOS, buildPath_);
            }

        }
        else
        {
            buildSystem->BuildComplete(CE_PLATFORM_IOS, buildPath_, false);
...
void BuildIOS::Build(const String& buildPath)
{
    ...
    String resourcePackagePath = buildDestDist + "/ClockworkResources.pak";
