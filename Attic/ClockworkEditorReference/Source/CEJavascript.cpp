...
WeakPtr<CEJavascript> CEJavascript::instance_;

CEJavascript::CEJavascript(Context* context) :
    Object(context)
{
    ...
    SubscribeToEvent(E_FILECHANGED, HANDLER(CEJavascript, HandleFileChanged));
    SubscribeToEvent(E_EDITORSHUTDOWN, HANDLER(CEJavascript, HandleEditorShutdown));

    ExecuteFile("ClockworkEditor/javascript/ClockworkEditor.js");
}

CEJavascript::~CEJavascript()
{
    duk_destroy_heap(ctx_);
    instance_ = NULL;
}

bool CEJavascript::ExecuteFile(const String& path)
{
...
bool CEJavascript::ParseJavascriptToJSON(const char* source, String& json, bool loose)
{
    json.Clear();
    ...
    duk_get_global_string(ctx_, loose ? "__clockwork_parse_to_json_loose" : "__clockwork_parse_to_json");
...
bool CEJavascript::BeautifyJavascript(const char* source, String& output)
{
    output.Clear();

    duk_get_global_string(ctx_, "__clockwork_js_beautify");
    duk_push_string(ctx_, source);
    bool ok = true;
...
bool CEJavascript::ReadZeroTerminatedSourceFile(const String& path, String& source)
{
...
int CEJavascript::js_read_source(duk_context* ctx)
{
...
int CEJavascript::js_module_search(duk_context* ctx)
{
    String path = duk_to_string(ctx, 0);

    path = "ClockworkEditor/javascript/modules/" + path + ".js";
...
void CEJavascript::HandleEditorShutdown(StringHash eventType, VariantMap& eventData)
{
